...
if __name__ == "__main__":
    ...
    sp = pd.read_csv("result/selective_p.csv", header=None).values[:, dim]
    nap = pd.read_csv("result/naive_p.csv", header=None).values[:, dim]
    ...
    plt.title("dim" + str(dim), fontsize=15)
    plt.savefig("fig/demo_dendrogram_p_dim" + str(dim) + ".svg", pad_inches=0, bbox_inches='tight')
    plt.show()