...
// PCI 各次元Ver 切断点保持
std::pair<std::vector<double>, std::vector<double>> PCI_ward_Degs_step(MatrixXd &data, std::vector<std::vector<ind>> &cluster_head_vec, std::vector<std::vector<ind>> &cluster_next_vec, std::vector<std::vector<ind>> &cluster_tail_vec, std::vector<std::pair<int, int>> &selected_c, MatrixXd &Sigma, MatrixXd &Xi, int step, int threads){
    int n = data.rows();
    int d = data.cols();
    ...
    // メインループ
    #pragma omp parallel for num_threads(threads)  // 並列化のためのオプション コンパイル時に -fopenmp をつけることで有効になる
